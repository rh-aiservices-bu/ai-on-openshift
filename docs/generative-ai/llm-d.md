...
LLM-D is a Kubernetes-native framework that speeds up distributed LLM inference at scale. It is able to orchestate several model server pods, allowing key optimisations so you can serve high scale production traffic efficiently and reliably.

### How does it work?

The core components can be seen in the architectual diagram below.
...
![image](img/llm-d-basic-architecture.svg)
  ...
  - LLM-D Router: The entrypoint for all inference requests.
    ...
    - LLM-D end point picker (EPP): The engine that will score and select model servers based on real-time metrics, KV-Cache affinity and other policies.
...
With these components, several key optimisations can be configured to significantly speed up inference request, including:
...
The prioritisation is determined by the authorization token in the request. This means that you must have enabled token-authorizaion for your LLM-D model deployment. From this token, the user and namespace that the request is coming from can be found, which is required for setting these headers.
